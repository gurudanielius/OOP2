...
//    outputFile.open("..\\Program Files (x86)\\Vilnius University\\" + file_name);
...
void output2(list<Stud> v, const string& file_name) {
    std::ofstream outputFile;
    std::string fullPath = "C:\\Program Files (x86)\\Vilnius University\\" + file_name;
    outputFile.open(fullPath);
    if (!outputFile.is_open()) {
        std::cerr << "Unable to open the file at " << fullPath << std::endl;
        return;
    }
    outputFile << left << setw(15) << "Vardas" << "    "
        << left << setw(15) << "Pavarde" << "    "
    ...
    outputFile.close();
    std::cout << file_name + " :Data written to the file successfully." << std::endl;
}
